//relationshipName补全
// 所有 main→relative 方向（main 是父母，relative 是子女）
MATCH (main:Patient)-[r:PARENT_OF]->(relative:Patient)
WHERE r.relationshipName IS NULL
SET r.relationshipName = '子女';

// 所有 relative→main 方向（relative 是父母）
MATCH (main:Patient)<-[r:PARENT_OF]-(relative:Patient)
WHERE r.relationshipName IS NULL
SET r.relationshipName = '父母';

// 所有 SPOUSE_OF
MATCH (:Patient)-[r:SPOUSE_OF]-(:Patient)
WHERE r.relationshipName IS NULL
SET r.relationshipName = '夫妻';
